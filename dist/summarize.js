function emptySideEffectCounts() {
...
export function summarize(cards, now) {
    ...
    for (const c of cards) {
        ...
        const piiExposure = c.safety.pii_exposure ?? "unset";
        ...
        if (sideEffect in bySideEffect)
            bySideEffect[sideEffect] += 1;
        if (piiExposure in byPiiExposure)
            byPiiExposure[piiExposure] += 1;
        if (sideEffect === "destructive")
            destructiveTools += 1;
        if (humanApproval)
            toolsRequiringApproval += 1;
        if (hasAudit)
            toolsWithAudit += 1;
        if (hasTestedWith)
            toolsWithTestedWith += 1;
        rows.push({
            id,
